...
sportModal=function(){
  const strokes={Crol:{met:8,pace:45},Espalda:{met:7,pace:40},Braza:{met:7,pace:32},Mariposa:{met:10,pace:28},'Piernas con tabla':{met:6.5,pace:30},'Nado suave':{met:6,pace:35}};
...
// Registro de sesión contextual: hereda la modalidad y permite varios estilos de natación.
sportModal=function(preselected){
  const strokes={Crol:{met:8,pace:45},Espalda:{met:7,pace:40},Braza:{met:7,pace:32},Mariposa:{met:10,pace:28},'Piernas con tabla':{met:6.5,pace:30},'Nado suave':{met:6,pace:35}};
  const sport=preselected||state.selectedSport||'Fuerza', swim=String(sport).toLowerCase().includes('nataci');
  modal(`<h2>Registrar ${sport}</h2><p class="notice">Usamos tu peso del perfil para estimar el gasto.</p><div class="pill">${sport.toUpperCase()}</div><label>Fecha<input id="sportDate" type="date" value="${window.calendarSelectedDate||new Date().toISOString().slice(0,10)}"></label><div id="sportExtra"></div><div class="input-row"><label>DuraciÃ³n (min)<input id="sportMin" type="number" min="1" value="60"></label><label>Intensidad<select id="sportIntensity"><option value="0.85">Suave</option><option value="1" selected>Media</option><option value="1.15">Alta</option></select></label></div><p id="sportEstimate" class="notice"></p><button class="button green wide" id="saveSport">AÃ±adir al progreso</button>`);
  const extra=document.querySelector('#sportExtra'),min=document.querySelector('#sportMin'),intensity=document.querySelector('#sportIntensity'),estimate=document.querySelector('#sportEstimate');
  const calc=()=>{let m=Number(min.value)||0,details={};if(swim){let kcal=0,meters=0;document.querySelectorAll('.swim-row').forEach(row=>{const s=row.querySelector('.swim-stroke').value,mm=Number(row.querySelector('.swim-meters').value)||0;meters+=mm;kcal+=(strokes[s]?.met||7)*3.5*(state.weight||70)/200*(mm/(strokes[s]?.pace||35));});m=Math.max(m,meters/35);details={swimBlocks:[...document.querySelectorAll('.swim-row')].map(row=>({stroke:row.querySelector('.swim-stroke').value,meters:Number(row.querySelector('.swim-meters').value)||0})),meters};const k=Math.round(kcal*Number(intensity.value));estimate.textContent=`EstimaciÃ³n: ${k} kcal Â· ${Math.round(m)} min Â· ${meters} m`;return {k,m,details};}const k=Math.round((sportMET[sport]||6)*3.5*(state.weight||70)/200*m*Number(intensity.value));estimate.textContent=`EstimaciÃ³n: ${k} kcal Â· ${m} min`;return {k,m,details};};
  if(swim){extra.innerHTML=`<h3>Bloques de nado</h3><div id="swimRows"></div><button class="tab wide" id="addSwim">＋ Añadir otro estilo</button>`;const rows=document.querySelector('#swimRows');const add=()=>{const row=document.createElement('div');row.className='input-row swim-row';row.innerHTML=`<label>Estilo<select class="swim-stroke">${Object.keys(strokes).map(x=>`<option>${x}</option>`).join('')}</select></label><label>Metros<input class="swim-meters" type="number" min="1" value="500"></label>`;rows.appendChild(row);row.querySelectorAll('input,select').forEach(x=>x.oninput=calc);};add();document.querySelector('#addSwim').onclick=add;}
